...
class Rating implements Comparable<Rating> {
    ...
    public int compareTo(Rating rating) {
        ...
        var currentNetRating = this.upVotes - this.downVotes;
        var anotherNetRating = rating.getUpVotes() - rating.getDownVotes();
        var equlity = 0;
        if (currentNetRating == anotherNetRating) {
            equlity = 0;
        } else if (currentNetRating > anotherNetRating) {
            equlity = 1;
        } else if (currentNetRating < anotherNetRating) {
            equlity = -1;
        }
